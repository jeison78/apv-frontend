...
const Registrar = () => {
  ...
  const handleSubmit = async e => {
    ...
    if (password !== repetirPassword) {
      setAlerta({ msg: 'Los password no sos iguales', error: true })
      return;
    }
    ...
    try {
      ...
      setAlerta({
        msg: 'Creando Correctamente, revisa tu email',
        error: false
      })
    } catch (error) {
      setAlerta({
    ...
    <>
      ...
      <div className="mt-10 md:mt-12 shadow-lg px-10 py-5 rounded-xl bg-white ">
        ...
        <nav className='mt-4 lg:flex lg:justify-between'>
          ...
          <Link className="block text-center my-5  text-rose-300" to="/olvide-password"> Olvide mi Password</Link>
        </nav>
      </div>
